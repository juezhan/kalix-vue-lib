  ...
  export default {
    ...
    data() {
      return {
        ...
        targetURL: this.$KalixGlobalConfig.AttachmentURL,
        jsonStr: '',
        bizKey: '',
        title: '',
        visible: false,
    ...
    methods: {
      ...
      deleteSelectFile(_index, row) {
        this.$confirm('确定要删除吗?', '提示', {
          ...
          return this.axios.request({
            method: 'DELETE',
            url: this.$KalixGlobalConfig.AttachmentURL + '/' + row.id,
            params: {},
            data: {
              id: row.id
            }
      ...
      _fileUpload(item, callBack) {
        this.$http.post(this.$KalixGlobalConfig.AttachmentURL, item).then(res => {
          if (res.data.success) {
            Message.success(res.data.msg)
