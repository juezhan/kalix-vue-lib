...
//        required: true
    ...
    data() {
      return {
        ...
        pager: {
          totalCount: 0,
          pageSizes: this.$KalixGlobalConfig.PageConfig.sizes,
          currentPage: 1,
      ...
      _validateButton() {
        if (this.buttonPermissionPrefix !== '') { // 默认为空不校验
          ...
          if (_permissionData.length > 0) {
            this.$http.get(`${this.$KalixGlobalConfig.SecurityBtnUrl}${_permissionData.join('_')}`).then(res => {
...
//          console.log(`[Kalix] table tool button list is `, this.btnList)
      ...
      _getTableHeight() {
        if (this.$refs.kalixTableContainer) {
